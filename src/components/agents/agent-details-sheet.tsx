...
export function AgentDetailsSheet({ agent, missions, isOpen, onOpenChange }: AgentDetailsProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl sm:max-w-md bg-background/80 backdrop-blur-md shadow-2xl border-white/30 max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
        ...
        <ScrollArea className="flex-1 pr-4">
          <div className="py-6 space-y-6">
              <div className="flex flex-col items-center text-center gap-4">
                  <div className="h-32 w-32 rounded-full border-4 border-muted overflow-hidden flex items-center justify-center bg-muted text-muted-foreground shadow-md">
                      {agent.photo ? (
                        <Image src={agent.photo} alt={agent.fullName} width={128} height={128} className="object-cover h-full w-full" />
                      ) : (
                  ...
                  <div>
                      <h2 className="text-2xl font-bold">{agent.fullName}</h2>
                       <p className="text-muted-foreground font-mono">{agent.registrationNumber}</p>
                  </div>
              </div>
              ...
              <div className="space-y-2 pt-4">
                  ...
                  <div className="border rounded-lg max-h-[15rem] overflow-hidden">
